...
interface ReceivedShoppingListInvitesModalProps {
  open: boolean;
  onClose: () => void;
  onUpdate: () => void;
}

export function ReceivedShoppingListInvitesModal({
  open,
  onClose,
  onUpdate,
}: ReceivedShoppingListInvitesModalProps) {
  const [invites, setInvites] = useState<ShoppingListInviteResponse[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await shoppingListApi.listReceivedInvites();
      setInvites(data);
    } catch (e) {
              ...
              <div className="flex flex-col items-center gap-4 py-10 text-center">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    Você não tem convites pendentes para listas de compras.
                  </p>
                </div>
              </div>
            )}
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-sm font-semibold text-slate-900 dark:text-white">
                              {invite.shopping_list_title ?? "Lista de compras"}
                            </p>
                          </div>
